...
//4 outputs
const int moveCartFwd = 13;   //C7	/* Change this to be just one pin. 
//const int moveCartBack = 13;   //C7	/* Change this to be just one pin. 
const int moveCartBack = 5;   //C6	/* Change this to be just one pin. 
//const int motorPower = 5;   //C6
...
void setup() {
	Serial.begin(115200);		/* What is this number for? */
	...
	while (autoState != STOP) {
		...
		switch (autoState) {
      	...
      	case ACTUATORS_OUT:
         	Serial.println("send act. out");
          	if (sendMessage(0x06) == 0x02) {
            	...
            	while (Serial1.available() == 0);		// IS THIS SUPPOSED TO BE LIKE THIS?
      	...
      	case ACTUATORS_IN:
            	...
            	while (Serial1.available() == 0);	// CHECK THAT THIS IS SUPPOSED TO BE LIKE THIS...
      	...
      	case CHECK_BATTERIES:
         	if (!frontChecked && (digitalRead(cartAtFront) == LOW)) { // CARTS @ FRONT
            	...
            	if ((backBatteryVoltage > BATTERY_VOLTAGE_MIN_DIFFERENCE) && (frontBatteryVoltage > BATTERY_VOLTAGE_MIN_DIFFERENCE))
              		error();
            	else if ((backBatteryVoltage <= BATTERY_VOLTAGE_MIN_DIFFERENCE) && (frontBatteryVoltage <= BATTERY_VOLTAGE_MIN_DIFFERENCE))
              		error();
...
void movToBack() {
	if (digitalRead(cartAtBack) == HIGH) {
   	digitalWrite(moveCartFwd, LOW);
    	digitalWrite(moveCartBack, HIGH);
...
void movToFront() {
	if (digitalRead(cartAtFront) == HIGH) {
   	digitalWrite(moveCartBack, LOW);
    	digitalWrite(moveCartFwd, HIGH);
